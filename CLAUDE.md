...
- `Resources/webpub/divina/`: DiViNa fixtures (folder, packaged as `.divina` by build script). Generated from `epub+audio` sources via `bin/divina_from_nota_comic.py`.
- `Resources/cbz/`: Comic Book ZIP source directories (packaged as `.cbz`).
- `Resources/generated/`: **Gitignored.** Pre-built CBZ archives generated by `bin/cbz_gen_sample.py`. Not committed; populate locally after cloning.
